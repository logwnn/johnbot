...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((o) =>
      o.setName("days").setDescription("Archive files older than N days").setRequired(false)
    ),
  async executeSlash(client, interaction) {
    const days = interaction.options.getInteger("days") || 7;
    await interaction.reply({
      content: `Archiving logs older than ${days} days...`,
      ephemeral: true,
    });
    try {
      ...
      logEvent("ERROR", `Archive logs failed | ${e.stack}`);
      try {
        await interaction.editReply({ content: `Failed to archive logs: ${e.stack}` });
      } catch {}
    }
  },
};
