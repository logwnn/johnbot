...
export default {
  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) => opt.setName("prompt").setDescription("Prompt to ask the model").setRequired(true)),
  async executeSlash(client, interaction) {
    try {
      const replyMessage = await interaction.reply({ content: config.thinkingReply, ephemeral: false, fetchReply: true });
      ...
      try {
        if (!interaction.replied) await interaction.reply({ content: "Failed to run askmodel.", ephemeral: true });
      } catch {}
    }
